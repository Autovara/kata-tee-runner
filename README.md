...
the whole set is sealed together, by the same tool in multi-key mode:
...
`--provider` and `--providers` are mutually exclusive and one is required. The mode is never
inferred, because the room dispatches on the sealed payload's version: a single-key credential where
the lane wants a set is refused mid-duel, and a credential failure scores zero.

A key is never accepted as a command-line **value** — only the name of an environment variable, a
...
| Tool | Purpose |
| --- | --- |
| `kata_seal.py` | seal one credential (v1) or a named set (v2) to a bundle |
| `verify_run.py` | check a returned report against its quote |

`kata_seal.py` flags — a documented flag the tool does not accept strands a miner at the one step
where they are handling real credentials:

`--room`, `--provider`, `--providers`, `--credential-profile`, `--key-env`, `--key-file`,
`--bundle`, `--measurement`, `--out`, `--no-verify`
